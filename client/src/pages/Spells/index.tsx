...
import { SpellsFilter, SpellsSort, SpellsSchool } from '../../lib/graphql/globalTypes';
...
export const Spells = ({ match }: RouteComponentProps<MatchParams>): JSX.Element => {
  ...
  if (match.params.filter) {
    const trimmedRouteParam = match.params.filter.trim();
    if (match.params.filter) {
  ...
  if (error) {
    displayErrorMessage(
      'Unable to load spells. Check your internet connection or try again later'
    );
    return <Redirect to="/" />;
  }
